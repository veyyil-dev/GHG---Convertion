...
//   console.log("Scope Two Total:", scopeTwoTotal);
  ...
  const TotalDataEntryLocal = (scopeTwoTotal) => {
  ...
  useEffect(() => {
    ...
    if (selectedValuesScopeTwo && Object.keys(selectedValuesScopeTwo).length > 0) {
      fetchParameters();
    } else {
      setIsLoading(false);
    }
  ...
  useEffect(() => {
    const templateSave = localStorage.getItem("templateSave_scope2");
    if (templateSave) {
      try {
        ...
        setSelectedFuels({}); // Set empty object if parsing fails
      }
    }
  }, []);
