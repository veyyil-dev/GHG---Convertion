...
export default function ParametersAndUnitsEdit() {
  // const {  setScopeOneTotal,userId } = useScopeOne();
  ...
  const TotalDateEntryLocal = (scopeOneTotal) => {
...
useEffect(() => {
  ...
  const fetchScopeData = async () => {
    try {
      const response = await fetch(`https://ghg-conversion-factors-backend.vercel.app/getAllentriesByidGet?allentries_id=${allentries_id}`);
  ...
  useEffect(() => {
    const templateSave = async () => {
      const response = await fetch(`https://ghg-conversion-factors-backend.vercel.app/getTempnameToEditing/ForEdit?template_id=${localStorage.getItem("template_id")}`);
      const data = await response.json();
      console.log("datafromparamt", data)
      setSelectedFuels(data[0])
    }
    templateSave();
  }, []);
  ...
  return (
    <>
      {/* <div className="page-header flex items-center gap-2  p-1 cursor-pointer">
        {/* <div type="primary" onClick={() => router.back()}><ArrowLeftOutlined className="text-2xl mb-3" /></div> */}
            ...
            <Disclosure key={category} defaultOpen>
              {({ open }) => (
                <div className="bg-[#BFF1DF] w-full mt-4 rounded-lg shadow-sm hover:shadow-md transition-shadow">
                  ...
                  >
                    <Disclosure.Panel className="p-4 w-full bg-[#effbf7] rounded-b-lg max-h-[300px] overflow-y-auto">
